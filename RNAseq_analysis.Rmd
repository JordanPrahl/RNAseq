...
```{r, Steve's code for formal relationships between groups}
pca <- prcomp(t(cpm(my_dge, log=TRUE)))
top5 <- pca$x[, 1:5]
design <- model.matrix(~0 + Condition + ..., data = samplekey)
fit <- lmFit(t(top5), design, method="robust", maxit=100)
fit <- ...contrasts..
decideTests(fit)
topTable(fit, coef=xxx) for each contrast
```
...
View(dat2)
dat3 <-cbind.data.frame(Geno, dat2)
View(dat3)
df <- data.frame(x = dat3[,2],
                 y = dat3[,3],
                 Geno = dat3$Geno)


p <- ggplot(data <- dat3, aes(x=dat3[,2], y=dat3[,3])) +
  geom_point(aes(color = Geno, size=10)) +
  ggtitle("TSNE of RNAseq data") +
...
library(M3C)
var <- seq(1, ncol(dat))
M3C::umap(dat, labels=var, controlscale=T, scale=1, legendtitle="var", printres=T, printwidth=24)
```